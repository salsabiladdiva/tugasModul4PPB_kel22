...
export default function RecipeGrid({ recipes, onNavigate }) {
  ...
  const toggleFavorite = (e, recipe) => {
    e.stopPropagation();
    const recipeId = `makanan-${recipe.id}`;
  ...
  return (
    <section>
      ...
      <p className="text-center text-slate-500 max-w-2xl mx-auto mb-8">
        Temukan inspirasi masakan Nusantara favoritmu. Dari hidangan utama
        hingga camilan, semua ada di sini.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {recipes.map((recipe, index) => (
          <div
            ...
            onClick={() => onNavigate?.("detail", `makanan-${recipe.id}`)}
          >
            <div className="relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl shadow-blue-500/5 hover:shadow-blue-500/15 transition-all duration-500 cursor-pointer group-hover:scale-105 group-hover:bg-white/20">
              ...
              <button
                ...
                aria-label={favorites.has(`makanan-${recipe.id}`) ? "Hapus dari Favorit" : "Tambah ke Favorit"}
              >
                <Heart
                  className={`w-5 h-5 transition-colors duration-200 ${
                    favorites.has(`makanan-${recipe.id}`) ? "text-red-500 fill-current" : "text-gray-400"
                  }`}
                />
              </button>
